...
type (
	...
	Groups struct {
		Id              string         `db:"id"`               // 群组ID
		Name            string         `db:"name"`             // 群组名称
		Icon            string         `db:"icon"`             // 群组头像
		Status          sql.NullInt64  `db:"status"`           // 群组状态
		CreatorUid      string         `db:"creator_uid"`      // 群主用户ID
		GroupType       int64          `db:"group_type"`       // 群组类型
		IsVerify        bool           `db:"is_verify"`        // 是否需要验证
		Notification    sql.NullString `db:"notification"`     // 群公告
		NotificationUid sql.NullString `db:"notification_uid"` // 发布群公告的用户ID
		CreatedAt       sql.NullTime   `db:"created_at"`       // 群组创建时间
		UpdatedAt       sql.NullTime   `db:"updated_at"`       // 群组更新时间
	}
)
