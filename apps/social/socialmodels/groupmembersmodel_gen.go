...
type (
	...
	GroupMembers struct {
		...
		RoleLevel   int64          `db:"role_level"`   // 角色级别（群主、管理员、普通成员）
		JoinTime    sql.NullTime   `db:"join_time"`    // 加入时间
		JoinSource  sql.NullInt64  `db:"join_source"`  // 加入来源（邀请、申请等）
		InviterUid  sql.NullString `db:"inviter_uid"`  // 邀请者用户ID
		OperatorUid sql.NullString `db:"operator_uid"` // 操作人用户ID（如移除成员的人）
	}
)
