...
type (
	...
	GroupRequests struct {
		...
		ReqId         string         `db:"req_id"`          // 请求用户ID
		GroupId       string         `db:"group_id"`        // 群组ID
		ReqMsg        sql.NullString `db:"req_msg"`         // 请求加入的消息
		ReqTime       sql.NullTime   `db:"req_time"`        // 请求时间
		JoinSource    sql.NullInt64  `db:"join_source"`     // 加入来源（申请、邀请等）
		InviterUserId sql.NullString `db:"inviter_user_id"` // 邀请用户ID
		HandleUserId  sql.NullString `db:"handle_user_id"`  // 处理请求的管理员ID
		HandleTime    sql.NullTime   `db:"handle_time"`     // 请求处理时间
		HandleResult  sql.NullInt64  `db:"handle_result"`   // 处理结果（同意、拒绝等）
	}
)
